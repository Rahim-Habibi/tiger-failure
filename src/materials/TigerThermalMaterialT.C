...
TigerThermalMaterialT::validParams()
{
  ...
  params.addRequiredParam<std::vector<Real>>(
      "lambda", "Initial thermal conductivity of rock matrix (W/(m K))");
  MooseEnum Advection("pure_diffusion darcy_velocity user_velocity darcy_user_velocities",
                      "darcy_velocity");
  params.addParam<MooseEnum>("advection_type",
                             Advection,
  ...
  params.addParam<MooseEnum>("mean_calculation_type",
                             Mean,
  ...
  params.addParam<bool>(
      "has_supg", false, "Is Streameline Upwinding / Petrov Galerkin (SU/PG) activated?");
  params.addParam<Real>(
      "supg_coeficient_scale", 1.0, "The user defined factor to scale SU/PG coefficent (tau)");
  params.addParam<FunctionName>(
      "user_velocity", 0.0, "a vector function to define the velocity field");
  params.addParam<UserObjectName>("supg_uo", "", "The name of the userobject for SU/PG");
...
{
  _Pe = (_has_PeCr || _has_supg) ? &declareProperty<Real>("thermal_peclet_number") : NULL;
  _Cr = (_has_PeCr || _has_supg) ? &declareProperty<Real>("thermal_courant_number") : NULL;
  _vel_func = (_at == AT::user_velocity || _at == AT::darcy_user_velocities)
                  ? &getFunction("user_velocity")
                  : NULL;
  _supg_uo = (parameters.isParamSetByUser("supg_uo")) ? &getUserObject<TigerSUPG>("supg_uo") : NULL;
  _dv = (_at == AT::darcy_velocity || _at == AT::darcy_user_velocities)
            ? &getMaterialProperty<RealVectorValue>("darcy_velocity")
            : NULL;
}
...
TigerThermalMaterialT::computeQpProperties()
{
  ...
  if (_has_supg)
  {
    // should be multiplied by the gradient of the test function to build the Petrov Galerkin P
    // function
    _supg_uo->SUPGCalculator(
        lambda, _dt, _current_elem, _av[_qp], _SUPG_p[_qp], (*_Pe)[_qp], (*_Cr)[_qp]);
...
RankTwoTensor
TigerThermalMaterialT::Ari_Cond_Calc(Real const & n,
                                     Real const & lambda_f,
                                     const std::vector<Real> & lambda_s,
                                     const int & dim)
{
  ...
  if (dim == 1)
    switch (_ct)
    {
      case CT::isotropic:
        if (lambda_s.size() != 1)
          mooseError("One input value is needed for isotropic distribution of thermal "
                     "conductivity! You provided ",
                     lambda_s.size(),
                     " values.\n");
      ...
      case CT::orthotropic... CT::anisotropic:
        mooseError(
            "One dimensional elements cannot have non-isotropic thermal conductivity values.\n");
        break;
    }
  else if (dim == 2)
    switch (_ct)
    {
      ...
      case CT::orthotropic:
        if (lambda_s.size() != 2)
          mooseError("Two input values are needed for orthotropic distribution of thermal "
                     "conductivity in two dimensional elements! You provided ",
                     lambda_s.size(),
                     " values.\n");
      ...
      case CT::anisotropic:
        if (lambda_s.size() != 4)
          mooseError("Four input values are needed for anisotropic distribution of thermal "
                     "conductivity in two dimensional elements! You provided ",
                     lambda_s.size(),
                     " values.\n");
    ...
    switch (_ct)
    {
      ...
      case CT::orthotropic:
        if (lambda_s.size() != 3)
          mooseError("Three input values are needed for orthotropic distribution of thermal "
                     "conductivity! You provided ",
                     lambda_s.size(),
                     " values.\n");
      ...
      case CT::anisotropic:
        if (lambda_s.size() != 9)
          mooseError("Nine input values are needed for anisotropic distribution of thermal "
                     "conductivity! You provided ",
                     lambda_s.size(),
                     " values.\n");
...
RankTwoTensor
TigerThermalMaterialT::Geo_Cond_Calc(Real const & n,
                                     Real const & lambda_f,
                                     const std::vector<Real> & lambda_s,
                                     const int & dim)
{
      ...
      case CT::isotropic:
        if (lambda_s.size() != 1)
          mooseError("One input value is needed for isotropic distribution of thermal "
                     ...
                     lambda_s.size(),
                     " values.\n");
        lambda = RankTwoTensor(1., 0., 0., 0., 0., 0.) * std::pow(lambda_f, n) *
                 std::pow(lambda_s[0], (1. - n));
        break;
      case CT::orthotropic... CT::anisotropic:
        mooseError(
            "One dimensional elements cannot have non-isotropic thermal conductivity values.\n");
      ...
      case CT::isotropic:
        if (lambda_s.size() != 1)
          mooseError("One input value is needed for isotropic distribution of thermal "
                     ...
                     lambda_s.size(),
                     " values.\n");
        lambda = RankTwoTensor(1., 1., 0., 0., 0., 0.) * std::pow(lambda_f, n) *
                 std::pow(lambda_s[0], (1. - n));
        break;
      case CT::orthotropic:
        if (lambda_s.size() != 2)
          mooseError("Two input values are needed for orthotropic distribution of thermal "
                     ...
                     lambda_s.size(),
                     " values.\n");
        lambda = RankTwoTensor(
            std::pow(lambda_s[0], (1. - n)), std::pow(lambda_s[1], (1. - n)), 0., 0., 0., 0.);
        lambda *= std::pow(lambda_f, n);
        break;
      case CT::anisotropic:
        mooseError("Geometric mean for thermal conductivity of mixture is not available in "
      ...
      case CT::isotropic:
        if (lambda_s.size() != 1)
          mooseError("One input value is needed for isotropic distribution of thermal "
                     ...
                     " values.\n");
        lambda =
            RankTwoTensor::Identity() * std::pow(lambda_f, n) * std::pow(lambda_s[0], (1. - n));
        break;
      case CT::orthotropic:
        if (lambda_s.size() != 3)
          mooseError("Three input values are needed for orthotropic distribution of thermal "
                     ...
                     lambda_s.size(),
                     " values.\n");
        lambda = RankTwoTensor(std::pow(lambda_s[0], (1. - n)),
                               std::pow(lambda_s[1], (1. - n)),
                               std::pow(lambda_s[2], (1. - n)),
                               0.,
                               0.,
                               0.);
        lambda *= std::pow(lambda_f, n);
